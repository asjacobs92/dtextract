...
class GaussianMixtureDist:
    def __init__(self, X, numComponents):
        self.gmm = GaussianMixture(n_components=numComponents, covariance_type='diag', reg_covar=1e-1)
        self.nCols = np.shape(X)[1]
        self.numComponents = numComponents
        # fit a Gaussian mixture model
        log('Fitting Gaussian mixture with {} components and {} columns ...'.format(str(numComponents), str(self.nCols)), INFO)
        self.gmm.fit(X)
        log('Done!', INFO)
...
class CategoricalGaussianMixtureDist:
    ...
    def sample(self, cons, nPts):
        ...
        if not self.GMDist is None:
            numeric_xs = self.GMDist.sample([limits[nF] for nF in self.numericFeatInds], nPts)
