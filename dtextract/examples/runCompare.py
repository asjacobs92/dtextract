...
def runCompareSingle(path, hasHeader, dataTypes, isClassify, nDataMatrixCols, distType, headers):
    ...
    dtExtract.plot("../res/{}_dtExtract".format(path.split('/')[-1]))
...
def runCompare(path, hasHeader, dataTypes, isClassify, nDataMatrixCols, output, distType="CategoricalGaussianMixture", nRepeats=1, headers=None):
    setCurOutput(output)
    ...
    for i in range(nRepeats):
        curVals = runCompareSingle(path, hasHeader, dataTypes, isClassify, nDataMatrixCols, distType, headers)
